...
def extract_sequences(genome, bed_file, out_fa_name, half_len = 15):
    ...
    ## start end is of the peak, not of the snp
    # Open the output FASTA file for writing
    with open(out_fa_name, "w") as out_fa:
        for i in range(len(bed)):
            row = bed.iloc[i]
            #alt_nuc = row.snp.split('_')[-1]
            chr, pos, ref, alt = row.snp.split('_')
            # SNP coords are 1-based. 
            seq = fasta.fetch(str(chr), int(pos)-1-half_len, int(pos)+half_len)
            tem = list(seq)
            tem[half_len] = alt

            # Ensure the reference sequence matches the expected nucleotide
            assert seq[half_len].upper() == ref, f"{row.snp}: Expected {ref} but found {seq[half_len].upper()} sequence: {seq}"
